...
# Avoid conflicts with local packages
import types

# Create a namespace for our git package to avoid conflict with GitPython
git_extraction = types.ModuleType('git_extraction')
sys.modules['git_extraction'] = git_extraction

from git.config import GitExtractionConfig
from git.git_extractor import GitDataExtractor
...
def main():
    ...
    try:
        ...
        if 'rate_limit_stats' in cache_stats:
            ...
            print(f"  • API requests: {rate_stats['total_requests']} total, {rate_stats['rate_limited_requests']} rate limited")
        ...
        print(f"  • Files: {summary['commits_file']}, {summary['prs_file']}, {summary['repos_file']}")
