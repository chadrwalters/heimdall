...
class ProcessingLimitsConfig(BaseSettings):
    ...
    diff_truncation_buffer: int = Field(
        default=1000, ge=100, le=10000, env="NS_DIFF_TRUNCATION_BUFFER"
    )
    ...
    thread_timeout_seconds: int = Field(
        default=300, ge=30, le=3600, env="NS_THREAD_TIMEOUT_SECONDS"
    )
    ...
    memory_warning_threshold: float = Field(
        default=0.8, ge=0.5, le=0.95, env="NS_MEMORY_WARNING_THRESHOLD"
    )

    # API limits
    api_rate_limit_requests_per_minute: int = Field(
        default=100, ge=10, le=1000, env="NS_API_RATE_LIMIT_RPM"
    )
    ...
    api_retry_delay_seconds: float = Field(
        default=1.0, ge=0.1, le=60.0, env="NS_API_RETRY_DELAY_SECONDS"
    )
...
class ValidationLimitsConfig(BaseSettings):
    ...
    prompt_input_max_length: int = Field(
        default=100000, ge=1000, le=1000000, env="NS_PROMPT_INPUT_MAX_LENGTH"
    )
    title_max_length: int = Field(default=500, ge=10, le=1000, env="NS_TITLE_MAX_LENGTH")
    description_max_length: int = Field(
        default=10000, ge=100, le=50000, env="NS_DESCRIPTION_MAX_LENGTH"
    )
    ...
    graphql_query_max_length: int = Field(
        default=10000, ge=100, le=100000, env="NS_GRAPHQL_QUERY_MAX_LENGTH"
    )
    graphql_max_complexity: int = Field(
        default=1000, ge=10, le=10000, env="NS_GRAPHQL_MAX_COMPLEXITY"
    )
    graphql_max_depth: int = Field(default=15, ge=3, le=50, env="NS_GRAPHQL_MAX_DEPTH")
    graphql_variable_max_length: int = Field(
        default=1000, ge=10, le=10000, env="NS_GRAPHQL_VARIABLE_MAX_LENGTH"
    )
...
class PricingConfig(BaseSettings):
    ...
    claude_input_price_per_1k: float = Field(
        default=0.003, ge=0.0, le=1.0, env="NS_CLAUDE_INPUT_PRICE_PER_1K"
    )
    claude_output_price_per_1k: float = Field(
        default=0.015, ge=0.0, le=1.0, env="NS_CLAUDE_OUTPUT_PRICE_PER_1K"
    )

    # Cost monitoring
    daily_cost_warning_threshold: float = Field(
        default=50.0, ge=1.0, le=1000.0, env="NS_DAILY_COST_WARNING_THRESHOLD"
    )
    monthly_cost_limit: float = Field(
        default=1000.0, ge=10.0, le=10000.0, env="NS_MONTHLY_COST_LIMIT"
    )
