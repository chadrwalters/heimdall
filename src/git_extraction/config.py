...
class GitExtractionConfig:
    ...
    def from_environment(cls) -> "GitExtractionConfig":
        ...
        config.max_commits_per_query = int(
            os.getenv("GIT_MAX_COMMITS", config.max_commits_per_query)
        )
        config.single_branch_clone = os.getenv("GIT_SINGLE_BRANCH", "true").lower() == "true"

        # Performance settings
        config.max_concurrent_repos = int(
            os.getenv("GIT_MAX_CONCURRENT", config.max_concurrent_repos)
        )
        ...
        # Rate limiting settings
        config.api_rate_limit_buffer = int(
            os.getenv("API_RATE_LIMIT_BUFFER", config.api_rate_limit_buffer)
        )
        config.initial_backoff_delay = float(
            os.getenv("API_INITIAL_BACKOFF", config.initial_backoff_delay)
        )
        ...
        config.sanitize_commit_messages = (
            os.getenv("GIT_SANITIZE_MESSAGES", "true").lower() == "true"
        )
        config.max_commit_message_length = int(
            os.getenv("GIT_MAX_MESSAGE_LENGTH", config.max_commit_message_length)
        )
