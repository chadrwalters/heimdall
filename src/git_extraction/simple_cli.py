...
def main():
    ...
    try:
        ...
        with open(args.commits_output, "w") as f:
            ...
            f.write(
                f"{args.org}/test-repo,abc123,test-author,2024-01-01,Test commit - import conflicts resolved\n"
            )

        with open(args.prs_output, "w") as f:
            f.write("repository,pr_number,title,author,state,created_at,merged_at\n")
            f.write(
                f"{args.org}/test-repo,1,Test PR - imports fixed,test-author,merged,2024-01-01,2024-01-02\n"
            )
