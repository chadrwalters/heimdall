...
class CircuitBreakerMonitor:
    ...
    def check_health(self) -> Dict[str, Any]:
        ...
        health_report["summary"] = {
            ...
            "degraded_breakers": [name for name, s in stats.items() if s.get("failure_rate", 0) > 0.3]
        }
    ...
    def get_health_trends(self, hours: int = 24) -> Dict[str, Any]:
        ...
        if recent_history:
            ...
            trends["total_failures"] = sum(
                h["summary"]["failed_calls"] for h in recent_history
            )
        
        # Sort most problematic breakers
        trends["most_problematic_breakers"] = dict(
            sorted(trends["most_problematic_breakers"].items(), 
    ...
    def _generate_recommendations(self, stats: Dict[str, Any]) -> List[str]:
        ...
        if failure_rate > 0.5:
            recommendations.append(f"High failure rate ({failure_rate:.2%}). Investigate root cause.")
